...
export const UserProfile = (): JSX.Element => {
  ...
  const isWriteActive = pathname.startsWith("/blog/create");
  const isSavedActive = pathname.startsWith("/me/readinglist");
  const isBlogsActive = pathname.startsWith("/me/blogs");

  return (
    <>
      {/* Write icon */}
      <div
        onClick={createBlogOnClick}
        className={`flex justify-between items-center gap-x-2 p-2 rounded-md cursor-pointer transition-all font-semibold
          ${isWriteActive ? "bg-[#445046] text-white shadow-lg scale-105" : "bg-white text-[#445046] hover:bg-[#9dd5ca] hover:text-[#445046] hover:scale-110"}`}
      >
        <FaPenNib size={14} />
      ...
      {/* Saved blogs icon */}
      <div
        onClick={viewSavedBlogOnClick}
        className={`flex justify-between items-center gap-x-2 p-2 rounded-md cursor-pointer transition-all font-semibold
          ${isSavedActive ? "bg-[#445046] text-white shadow-lg scale-105" : "bg-white text-[#445046] hover:bg-[#9dd5ca] hover:text-[#445046] hover:scale-110"}`}
      >
      ...
      {/* My blogs icon */}
      <div
        onClick={viewMyBlogsOnClick}
        className={`flex justify-between items-center gap-x-2 p-2 rounded-md cursor-pointer transition-all font-semibold
          ${isBlogsActive ? "bg-[#445046] text-white shadow-lg scale-105" : "bg-white text-[#445046] hover:bg-[#9dd5ca] hover:text-[#445046] hover:scale-110"}`}
      >
      ...
      <div ref={dropdownRef} onClick={handleOptionsOnClick} className="flex justify-between gap-x-2 items-center p-2 cursor-pointer relative">
        <img src={`http://localhost:8000${userData?.profile_image}`} alt="profileimage" className="w-10 h-10 rounded-full" />
