    ...
    <div className="mx-auto max-w-2xl mt-24 w-full bg-white/90 rounded-2xl shadow-2xl border border-[#9dd5ca] p-6 backdrop-blur-md">
      <form className="w-full h-full flex flex-col gap-6">
        ...
        <div className="w-full">
          <ReactQuill theme="snow" modules={reactQuillModules} value={content} onChange={setContent} placeholder="Start writing :) ..." preserveWhitespace className="rounded-xl border border-[#9dd5ca] bg-white/70 min-h-[200px] focus:border-[#445046] transition-all duration-200" />
        </div>
        ...
        <div className="flex justify-center">
          <div className="flex justify-between gap-x-6 items-center w-full">
            ...
            >
              {isLoading ? <ClipLoader color="#445046" loading={isLoading} size={18} aria-label="loading-spinner" data-testid="loader" /> : "Save as Draft"}
            </button>
            <button
              type="button"
              onClick={(e) => {
            ...
            >
              {isLoading ? <ClipLoader color="#fff" loading={isLoading} size={18} aria-label="loading-spinner" data-testid="loader" /> : "Publish"}
            </button>
          </div>
        </div>
      </form>
